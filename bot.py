...
def reply_keyboard():
    return ReplyKeyboardMarkup([
        ...
        ["🗓 Подбор благоприятных дат", "🗂 Мои карты"],
...
def main_inline():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🪐 Транзиты планет",     callback_data="transit_today")],
        ...
        [InlineKeyboardButton("🗓 Подбор благоприятных дат", callback_data="muhurta")],
...
async def text_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif text == "🗓 Подбор благоприятных дат":
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "transit_today":
        ...
        t = get_detailed_positions(); ctx = get_transit_context()
        today = datetime.now().strftime("%d.%m.%Y")
        text = f"🪐 *ТРАНЗИТЫ И ПЛАНЕТЫ — {today}*\n_(сидерический зодиак, аянамша Лахири)_\n\n{t}"
        if ctx: text += f"\n\n📌 *Конфигурации:*\n{ctx}"
    ...
    elif data.startswith("geo_select:"):
        idx = int(data.split(":")[1])
        variants = context.user_data.get("geo_variants", [])
        if idx < len(variants):
            context.user_data["birth_place"] = variants[idx]["name"]
        await q.edit_message_text(f"📍 Выбрано: {context.user_data['birth_place']}")
        return await _finish_natal_calc_from_callback(q, context)
...
WAITING_PLACE_CONFIRM = 5

async def receive_birth_place(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid   = update.effective_user.id
    place = update.message.text.strip()

    # Геокодируем и предлагаем варианты
    from geopy.geocoders import Nominatim
    geo = Nominatim(user_agent="jyotish_bot_v4", timeout=10)
    try:
        results = geo.geocode(place, exactly_one=False, limit=4, language="ru", addressdetails=True)
    except Exception:
        results = None

    if results and len(results) > 1:
        # Предлагаем варианты
        context.user_data["geo_variants"] = [
            {"name": r.address, "lat": r.latitude, "lon": r.longitude}
            for r in results
        ]
        keyboard = []
        for i, r in enumerate(results):
            # Берём короткое название
            addr_parts = r.address.split(",")
            short = ", ".join(addr_parts[:3]).strip()
            keyboard.append([InlineKeyboardButton(short, callback_data=f"geo_select:{i}")])
        keyboard.append([InlineKeyboardButton("✏️ Ввести другое место", callback_data="geo_retry")])
        await update.message.reply_text(
            f"📍 Найдено несколько вариантов для *{place}*\nВыбери нужный:",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
    ...
    return await _finish_natal_calc(update, context)

async def _finish_natal_calc(update, context):
    uid = update.effective_user.id
    ...
    await update.message.reply_text("⏳ Рассчитываю карту...")
    natal = calculate_natal_basics(birth_date, birth_time, place)

    if natal.get("error"):
        await update.message.reply_text(f"❌ Ошибка: {natal['error']}\nПопробуй ещё раз.")
        return WAITING_BIRTH_PLACE
    ...
    # Показываем карту и спрашиваем сохранить ли
    text = (
        f"🔱 *Карта рассчитана*\n\n"
        f"👤 {person_name}\n"
        ...
        f"Сохранить карту в *Мои карты*?"
    )
    await update.message.reply_text(
        text, parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Сохранить", callback_data="save_card_yes"),
             InlineKeyboardButton("❌ Не сохранять", callback_data="save_card_no")],
        ])
    )
    return WAITING_SAVE_CONFIRM

async def _finish_natal_calc_from_callback(q, context):
    """Завершает расчёт карты из callback (после выбора геолокации)."""
    uid = q.from_user.id
    birth_date = context.user_data["birth_date"]
    birth_time = context.user_data.get("birth_time")
    person_name = context.user_data.get("person_name","Без имени")
    ...
    await q.message.reply_text("⏳ Рассчитываю карту...")
    from jyotish import calculate_natal_basics
    natal = calculate_natal_basics(birth_date, birth_time, place)

    if natal.get("error"):
        await q.message.reply_text(f"❌ Ошибка: {natal['error']}\nПопробуй ещё раз.")
        return WAITING_BIRTH_PLACE
...
async def receive_varsha_year(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ai = await call_ai(build_system_prompt(card), f"Интерпретируй Варшапхалу:\n{result}", uid)
...
def main():
    ...
    natal_conv = ConversationHandler(
        ...
        states={
            ...
            WAITING_PLACE_CONFIRM:[CallbackQueryHandler(button_handler, pattern="^geo_")],
        },
    ...
    app.add_handler(varsha_conv, group=-1)
    app.add_handler(muhurta_conv, group=-1)
    app.add_handler(compat_conv, group=-1)
