...
def calculate_compatibility(card1: dict, card2: dict) -> str:
    try:
        ...
        verd="🟢 Отличная" if pct>=75 else ("🟡 Хорошая" if pct>=60 else ("🟠 Средняя" if pct>=40 else "🔴 Низкая"))
        lines += [f"\n*── ИТОГ ──*",f"🏆 *{total}/36 ({pct}%)*",verd]
        if ns==0: lines.append("⚠️ *Нади-доша*")
        if bs==0: lines.append("⚠️ *Бхакута-доша*")
        return "\n".join(lines)
    except Exception as e:
